...
c:/OpenSSH/ssh vagrant@sshd.example.com 'whoami /all'

Write-Output 'Sending a file to the sshd host with the native scp client...'
c:/OpenSSH/scp native.ps1 vagrant@sshd.example.com:/tmp/example-powershell-native-scp.ps1

Write-Output 'Sending a file to the sshd host with the native sftp client...'
Write-Output 'put native.ps1 /tmp/example-powershell-native-sftp.ps1' | c:/OpenSSH/sftp -b - vagrant@sshd.example.com
